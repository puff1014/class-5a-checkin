...
const APP_VERSION = "v14.9.260222_Final_Polish";
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const fetchMonth = async () => {
      ...
      const stats = {};
      STUDENTS.forEach(s => stats[s.id] = { onTime: 0, late: 0, absent: 0, fullDoneDays: 0, lateDays: 0, missingDays: 0, issues: [] });
      ...
      for (const dKey of targetDates) {
        ...
        attSnap.forEach(doc => {
          ...
          else if (finalAtt === 'sick' || finalAtt === 'personal') stats[sid].absent++;
          ...
          if (dailyTasks.length > 0) {
            ...
            dailyTasks.forEach(t => {
              const finalTask = getFinalTaskStatus(sid, t, d);
              if (finalTask === 'missing') {
                missingCount++;
                stats[sid].issues.push(`${dKey.slice(5)}: ${t.trim()} (缺交)`);
              } else if (finalTask === 'late') {
                lateCount++;
                stats[sid].issues.push(`${dKey.slice(5)}: ${t.trim()} (遲交)`);
              }
            });
  ...
  const getStatusDisplay = (status, type) => {
    if (type === 'att') {
      switch(status) {
        ...
        case 'sick': return <span className="bg-slate-200 text-slate-700 px-3 py-1 rounded-lg">病假</span>;
      ...
      switch(status) {
        ...
        case 'exempt': return <span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-lg border border-purple-200">免交</span>;
    ...
    <div className="min-h-screen bg-[#F0F9FF] flex flex-col font-sans select-none overflow-x-hidden">
      ...
      <main className="flex-1 flex overflow-hidden p-4 h-[calc(100vh-250px)] print:hidden relative">
        ...
        <div style={{ width: `${w1}%` }} className="bg-white rounded-[3rem] shadow-sm p-4 flex flex-col border border-sky-50 h-full overflow-hidden shrink-0">
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-5 flex-1 overflow-y-auto pr-1 custom-scrollbar content-start">
            {STUDENTS.map(s => {
        ...
        <div style={{ width: `${w2}%` }} className="bg-white rounded-[3rem] shadow-sm p-4 flex flex-col border border-sky-50 h-full overflow-hidden shrink-0">
          ...
          <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar space-y-3 content-start">
            {STUDENTS.map(s => {
              const d = attendance[s.id];
        ...
        {/* 3. 任務區 - 改為 flex-1 自動填滿剩餘空間，解決邊界消失 */}
        <div className="flex-1 bg-[#0C4A6E] rounded-[3rem] shadow-xl p-8 text-white flex flex-col h-full overflow-hidden shrink-0">
          <div className="flex justify-between items-center mb-4 border-b border-white/20 pb-4 shrink-0">
            <h2 className="text-4xl font-black flex items-center gap-4 text-sky-400 drop-shadow-md"><ScrollText size={48}/> 任務發布區</h2>
            <div className="flex items-center gap-4">
          ...
          <div className="flex-1 bg-black/20 rounded-[2.5rem] p-8 overflow-y-auto custom-scrollbar border border-white/5 shadow-inner">
            {isEditing ? (
              <textarea value={announcementText} onChange={e => setAnnouncementText(e.target.value)} 
                ...
                className="flex-1 bg-transparent text-white font-black outline-none leading-relaxed text-4xl w-full min-h-[350px]" />
            ) : (
              <div style={{ fontFamily: useBiauKai ? '"BiauKai", "DFKai-SB", "標楷體", serif' : 'inherit', fontSize: `${fontSize}px`, lineHeight: lineHeight, fontWeight: '900' }}>
          ...
          <table className="w-full text-center table-fixed border-collapse">
            <thead className="sticky top-0 z-40 bg-sky-700 text-white shadow-md">
              <tr className="text-2xl font-black">
                ...
                <th className="p-4 bg-cyan-600 border-r border-sky-500 w-80">出席狀況</th>
                <th className="p-4 bg-indigo-600 border-r border-indigo-500">齊全</th>
                <th className="p-4 bg-amber-500 border-r border-amber-400">遲交</th>
                <th className="p-4 bg-rose-500">缺交</th>
              </tr>
            </thead>
                  ...
                  <tr key={s.id} className="hover:bg-sky-50/50 transition-colors">
                    ...
                    <td className="p-4 border-r-2 border-sky-50">
                      <div className="flex justify-center items-center gap-6 text-2xl font-black">
                        ...
                        <div className="flex items-center gap-2 text-slate-400"><UserMinus size={28}/> 假別: {sData ? sData.absent : '--'}</div>
                      </div>
                    </td>
                    <td className="p-4 text-sky-600 text-3xl font-black border-r-2 border-sky-50"><Trophy size={32} className="inline mr-2"/> {sData ? sData.fullDoneDays : '--'} 天</td>
                    <td className="p-4 text-amber-500 text-3xl font-black border-r-2 border-sky-50"><Clock size={32} className="inline mr-2"/> {sData ? sData.lateDays : '--'} 天</td>
                    <td className="p-4 text-rose-500 text-3xl font-black"><AlertTriangle size={32} className="inline mr-2"/> {sData ? sData.missingDays : '--'} 天</td>
                  </tr>
                );
              })}
            </tbody>
              ...
              {viewOnlyStudent && user ? (
                <div className="flex flex-col gap-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {prevTasks.map((t, idx) => {
                      ...
                      const fStat = getFinalTaskStatus(viewOnlyStudent.student.id, t, d);
                      const isManual = !!d?.manualTasks?.[t.trim()];
                      ...
                      return (
                        <div key={idx} className="bg-white border-4 border-slate-100 rounded-[2rem] p-6 flex justify-between items-center shadow-sm hover:border-sky-200 transition-colors">
                          <span className="text-4xl font-black text-slate-800 truncate pr-4">{t}</span>
                          <button onClick={() => cycleManualTask(viewOnlyStudent.student.id, t)} className="flex items-center gap-3 text-2xl font-black shrink-0 transition-transform active:scale-95 hover:opacity-80">
                ...
                <div className="grid grid-cols-3 gap-6">
                  {activeStudent ? prevTasks.map((t, idx) => (
                    <label key={idx} className={`p-6 rounded-[2rem] border-4 flex items-center gap-6 transition-all active:scale-95 cursor-pointer shadow-sm ${selectedTasks[t] ? 'bg-emerald-50 border-emerald-500 text-emerald-800 shadow-inner' : 'bg-white border-slate-300 text-slate-700 hover:border-emerald-300 hover:bg-slate-50'}`}>
      ...
      <div className="hidden print:block p-8 bg-white text-black font-sans">
        ...
        <div className="grid grid-cols-2 gap-8">
          {STUDENTS.map(s => {
            const sd = monthlyStats[s.id] || { onTime: 0, late: 0, absent: 0, fullDoneDays: 0, issues: [] };
            return (
              <div key={s.id} className="border-2 border-black p-6 rounded-xl break-inside-avoid">
                ...
                <div className="space-y-2 mb-4 text-lg">
                  ...
                  <p className="pl-6">準時 {sd.onTime} 天 / 遲到 {sd.late} 天 / 假別 {sd.absent} 天</p>
                  <p>● <span className="font-bold">作業：</span></p>
                  <p className="pl-6">齊全 {sd.fullDoneDays} 天</p>
                </div>
