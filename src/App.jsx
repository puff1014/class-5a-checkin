...
const APP_VERSION = "V21.3.260225_Absolute_Sync_Structure";
    ...
    <div className="min-h-screen bg-[#F0F9FF] flex flex-col font-sans select-none overflow-x-hidden">
          ...
          <div className="flex justify-between items-center mb-6 border-b border-white/20 pb-4 shrink-0 relative z-10">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 bg-white/10 p-2 rounded-2xl shadow-inner border border-white/10">
                <button onClick={() => setUseBiauKai(!useBiauKai)} className={`p-2 rounded-xl ${useBiauKai ? 'bg-sky-500 shadow-lg' : 'text-sky-200'}`} title="切換標楷體"><Type size={24}/></button>
                <div className="w-px h-6 bg-white/20 mx-1" /><button onClick={() => setFontSize(f => Math.max(20, f-4))} className="p-2 text-sky-100"><Minus/></button><button onClick={() => setFontSize(f => Math.min(100, f+4))} className="p-2 text-sky-100"><Plus/></button>
                <div className="w-px h-6 bg-white/20 mx-1" /><button onClick={() => setLineHeight(l => Math.max(0.7, l-0.1))} className="p-2 text-sky-100"><ArrowDown size={24}/></button><button onClick={() => setLineHeight(l => Math.min(3.0, l+0.1))} className="p-2 text-sky-100"><ArrowUp size={24}/></button>
              </div>
              {user && <button onClick={() => isEditing ? (setIsEditing(false), setDoc(doc(db, "announcements", formatDate(viewDate)), { items: announcementText.split('\n').filter(Boolean).map(t=> ({ text: t.trim(), colorIdx: 0 })), date: formatDate(viewDate) }, {merge:true})) : setIsEditing(true)} className="bg-emerald-500 px-8 py-3 rounded-2xl font-black text-2xl shadow-lg text-white active:scale-95">{isEditing ? '儲存任務' : '編輯任務'}</button>}
            </div>
          </div>
          {isEditing && (
            <div className="flex flex-wrap gap-2 mb-4 animate-fade-in pr-2 shrink-0 relative z-10">
              {QUICK_TAGS.map(t => <button key={t} onClick={() => setAnnouncementText(p => p ? p + '\n' + t : t)} className="px-4 py-2 bg-white/10 border border-white/20 rounded-xl text-xl font-bold hover:bg-white/30">{t}</button>)}
            </div>
          )}
      ...
      <section className="mx-4 mb-12 bg-white rounded-[3rem] p-8 shadow-2xl border-4 border-sky-100 flex flex-col print:hidden">
        <div className="flex justify-between items-center mb-6 px-2">
          ...
          {user && <button onClick={() => window.print()} className="bg-indigo-600 text-white px-6 py-2.5 rounded-2xl font-black text-xl active:scale-95 transition-all"><Printer size={24}/></button>}
        </div>
              ...
              {STUDENTS.map(s => {
                ...
                return (
                  <tr key={s.id} className="hover:bg-sky-50/50 transition-colors cursor-pointer group" onClick={() => sData && setViewOnlyStudent({ student: s, isHistory: true })}>
                    ...
                    <td className="p-5 border-r-2 border-sky-50"><div className="flex justify-center items-center gap-6 text-2xl font-black"><div className="text-emerald-600">準時: {sData ? sData.onTime : '--'}</div><div className="text-pink-500">遲到: {sData ? sData.late : '--'}</div></div></td>
                    <td className="p-5"><div className="flex justify-center items-center gap-10 text-2xl font-black"><div className="text-blue-600">齊全: {sData ? sData.fullDoneDays : '--'}</div><div className="text-rose-700">缺交: {sData ? sData.missingDays : '--'}</div></div></td>
                  </tr>
                );
              })}
            </tbody>
