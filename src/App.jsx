...
const APP_VERSION = "V21.1.260225_Cloud_Highlighter_Sync";
...
const App = () => {
  ...
  // 修改：將顏色更新同步至 Firebase
  const cycleHighlighter = async (index) => {
    if (!user) return; // 僅教師模式可改色
    ...
    const currentColorIdx = currentItem.colorIdx || 0;
    const nextColorIdx = (currentColorIdx + 1) % highlighterColors.length;
    
    newItems[index] = { text, colorIdx: nextColorIdx };
  ...
  useEffect(() => {
    ...
    const fetchMonth = async () => {
      ...
      const stats = {};
      STUDENTS.forEach(s => stats[s.id] = { onTime: 0, late: 0, sick: 0, personal: 0, fullDoneDays: 0, lateDays: 0, missingDays: 0, dailyRecords: {} });
      ...
      for (const dKey of targetDates) {
        ...
        STUDENTS.forEach(student => {
          ...
          const d = attMap[sid];
          if (!d) {
            stats[sid].dailyRecords[dKey] = { att: 'absent', missingList: dailyTasks.map(t=> typeof t === 'string' ? t.trim() : t.text.trim()), lateList: [], allDone: false };
          ...
          dailyTasks.forEach(t => {
            ...
            if (stat === 'missing') missing.push(tName);
            else if (stat === 'late') late.push(tName);
          });
          stats[sid].dailyRecords[dKey] = { att: finalAtt, missingList: missing, lateList: late, allDone: missing.length === 0 && late.length === 0 };
          if (missing.length > 0) stats[sid].missingDays++;
          else if (late.length > 0) stats[sid].lateDays++;
        ...
        <div style={{ width: `${w1}%` }} className="bg-white rounded-[3rem] shadow-sm p-5 flex flex-col border border-sky-50 shrink-0">
          ...
          <div className="grid grid-cols-2 gap-4 flex-1">
            {STUDENTS.map(s => {
              ...
              else if (attStat === 'sick' || attStat === 'personal') color = 'bg-purple-50 text-purple-600 border-purple-100';
              return (
                <button key={s.id} onClick={() => { setSelectedTasks(d?.completedTasks || {}); setActiveStudent(s); }} className={`min-h-[96px] rounded-[1.8rem] flex flex-col items-center justify-center transition-all border-b-8 active:border-b-0 ${color}`}><span className="text-5xl font-black">{maskName(s.name)}</span></button>
              );
            })}
          </div>
        </div>
        ...
        <div className="flex-1 bg-[#0C4A6E] rounded-[3rem] shadow-xl p-8 text-white flex flex-col shrink-0 min-w-0 relative overflow-hidden">
          ...
          <div className="flex justify-between items-center mb-6 border-b border-white/20 pb-4 shrink-0 relative z-10">
            ...
            <div className="flex items-center gap-4">
              ...
              {user && <button onClick={() => isEditing ? (setIsEditing(false), setDoc(doc(db, "announcements", formatDate(viewDate)), { items: announcementText.split('\n').filter(Boolean).map(t=> ({ text: t.trim(), colorIdx: 0 })), date: formatDate(viewDate) }, {merge:true})) : setIsEditing(true)} className="bg-emerald-500 hover:bg-emerald-400 px-8 py-3 rounded-2xl font-black text-2xl shadow-lg active:scale-95 text-white">{isEditing ? '儲存任務' : '編輯任務'}</button>}
            </div>
          </div>
          {isEditing && (
              ...
              {STUDENTS.map(s => {
                ...
                return (
                  <tr key={s.id} className="hover:bg-sky-50/50 transition-colors cursor-pointer group" onClick={() => sData && setViewOnlyStudent({ student: s, isHistory: true })}>
                    ...
                    <td className="p-5 border-r-2 border-sky-50"><div className="flex justify-center items-center gap-6 text-2xl font-black"><div className="text-emerald-600">準時: {sData ? sData.onTime : '--'}</div><div className="text-pink-500">遲到: {sData ? sData.late : '--'}</div><div className="text-slate-400">未到: {sData ? (sData.sick + sData.personal) : '--'}</div></div></td>
                    <td className="p-5"><div className="flex justify-center items-center gap-10 text-2xl font-black"><div className="text-blue-600">齊全: {sData ? sData.fullDoneDays : '--'}</div><div className="text-rose-500">遲交: {sData ? sData.lateDays : '--'}</div><div className="text-rose-700">缺交: {sData ? sData.missingDays : '--'}</div></div></td>
                  </tr>
                );
              })}
            </tbody>
