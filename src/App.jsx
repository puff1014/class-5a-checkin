...
import { Ship, ScrollText, ChevronLeft, ChevronRight, XCircle, Clock, UserCheck, Plus, Minus, Trash2, LayoutDashboard, Calendar, Trophy, XOctagon, CheckCircle2, Smile, Lock, Unlock, ArrowUp, ArrowDown, Printer } from 'lucide-react';

const APP_VERSION = "v14.6.260222_Ultimate_Master";
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const fetchMonth = async () => {
      ...
      const stats = {};
      STUDENTS.forEach(s => stats[s.id] = { onTime: 0, late: 0, absent: 0, fullDone: 0 });
      ...
      for (const dKey of targetDates) {
        ...
        attSnap.forEach(doc => {
          ...
          if (dailyTasks.length > 0) {
            ...
            if (dailyTasks.every(t => hw[t])) stats[sid].fullDone++;
          }
        });
      }
  ...
  }, [db, activeStatMonth, recordedDates]);

  const getAttStatus = (id, time) => {
    if (!time) return 'absent';
    const [h, m, s] = time.split(':').map(Number);
    const totalS = h * 3600 + m * 60 + (s || 0);
    if (SPECIAL_IDS.includes(id)) return totalS <= 8 * 3600 + 10 * 60 ? 'on-time' : 'late';
  ...
  return (
    <div className="min-h-screen bg-[#F0F9FF] flex flex-col font-sans select-none overflow-x-hidden">
      ...
      <main className="flex-1 flex overflow-hidden p-4 gap-4 h-[calc(100vh-250px)]">
        {/* 簽到區 - 高度解耦，內部滾動 */}
        <div className="w-[25%] bg-white rounded-[3rem] shadow-sm p-4 flex flex-col border border-sky-50 h-full overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-5 flex-1 overflow-y-auto pr-1 custom-scrollbar">
            {STUDENTS.map(s => {
              ...
              else if (d?.status === 'sick' || d?.status === 'personal') color = 'bg-slate-500 text-white border-slate-600 shadow-inner';
        ...
        {/* 進度區 - 高度解耦 */}
        <div className="w-[25%] bg-white rounded-[3rem] shadow-sm p-4 flex flex-col border border-sky-50 h-full overflow-hidden mx-2">
        ...
        {/* 任務發布區 - 教師編輯、標籤、行高回歸 */}
        <div className="flex-1 bg-[#0C4A6E] rounded-[3rem] shadow-xl p-8 text-white flex flex-col h-full overflow-hidden">
          <div className="flex justify-between items-center mb-4 border-b border-white/20 pb-4 shrink-0">
            ...
            <div className="flex items-center gap-4">
              ...
              {user && <button onClick={() => isEditing ? (setIsEditing(false), setDoc(doc(db, "announcements", formatDate(viewDate)), { items: announcementText.split('\n').filter(Boolean), date: formatDate(viewDate) }, {merge:true})) : setIsEditing(true)} className="bg-emerald-500 px-8 py-3 rounded-2xl font-black text-2xl shadow-lg transition-transform active:scale-95">{isEditing ? '儲存' : '編輯'}</button>}
            </div>
          </div>
          {isEditing && (
      ...
      {/* 每月分析報表 - 背景靜默計算、字體縮小、選單回歸 */}
      <section className="mx-4 mb-8 bg-white rounded-[3rem] p-6 shadow-2xl border-4 border-sky-100 flex flex-col shrink-0 min-h-[300px]">
        <div className="flex justify-between items-center mb-4 px-2">
          <h3 className="text-4xl font-black text-sky-900 flex items-center gap-5"><Calendar size={48} className="text-sky-600"/> 每月分析報表</h3>
          <div className="flex gap-4 items-center">
            <select value={activeStatMonth} onChange={(e) => setActiveStatMonth(e.target.value)} className="bg-sky-50 border-2 border-sky-200 text-sky-700 rounded-2xl px-6 py-2 font-black text-2xl outline-none shadow-sm cursor-pointer">
              {["2月", "3月", "4月", "5月", "6月", "7月"].map(m => <option key={m} value={m}>{m} 統計</option>)}
            </select>
            {user && <button onClick={() => window.print()} className="flex items-center gap-3 bg-indigo-600 text-white px-6 py-2.5 rounded-2xl font-black text-xl hover:bg-indigo-500 shadow-xl transition-all"><Printer size={24}/> 列印報表</button>}
          </div>
        </div>
        <div className="flex-1 overflow-auto rounded-[2rem] border-2 border-sky-50 relative custom-scrollbar">
          <table className="w-full text-center table-fixed border-collapse">
            <thead className="sticky top-0 z-40 bg-sky-700 text-white shadow-md">
              <tr className="text-2xl font-black"><th className="p-4 bg-sky-800 border-r border-sky-600 sticky left-0 z-50 w-64 text-left pl-10">姓名</th><th className="p-4 bg-cyan-600 border-r border-sky-500">出席狀況</th><th className="p-4 bg-indigo-600">作業繳交狀況</th></tr>
            </thead>
                  ...
                  <tr key={s.id} className="hover:bg-sky-50/50 transition-colors">
                    ...
                    <td className="p-4 border-r-2 border-sky-50">
                      <div className="flex justify-center items-center gap-8 text-2xl font-black">
                        ...
                        <div className="flex items-center gap-2 text-slate-400"><UserCheck size={32}/> 假別: {sData ? sData.absent : '--'}</div>
                      </div>
                    </td>
          ...
          <div className="bg-white rounded-[4rem] w-full max-w-[90vw] p-10 shadow-2xl relative flex flex-col max-h-[90vh] border-[12px] border-sky-100/50">
            <div className="flex justify-between items-center mb-6 border-b-4 border-sky-50 pb-6 shrink-0">
              <h3 className="text-6xl font-black text-sky-900 leading-none">
                ...
                <span className="text-2xl text-sky-400 ml-6 font-bold tracking-widest bg-sky-50 px-4 py-2 rounded-full">任務確認 - {formatDate(viewDate)}</span>
              </h3>
            ...
            <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar">
              <div className="grid grid-cols-3 gap-6">
                {activeStudent ? prevTasks.map((t, idx) => (
                  <label key={idx} className={`p-6 rounded-[2rem] border-4 flex items-center gap-6 transition-all active:scale-95 cursor-pointer shadow-sm ${selectedTasks[t] ? 'bg-sky-50 border-sky-500 shadow-inner' : 'bg-slate-50 border-slate-200 hover:border-sky-300'}`}>
                    <input type="checkbox" checked={!!selectedTasks[t]} onChange={(e) => setSelectedTasks({...selectedTasks, [t]: e.target.checked})} className="w-10 h-10 accent-sky-600 cursor-pointer" />
                  ...
                  <div className="col-span-3 flex flex-col items-center justify-center py-10 w-full h-full">
                    {prevTasks.length > 0 && prevTasks.every(t => viewOnlyStudent.tasks[t]) ? (
                      ...
                      <div className="grid grid-cols-3 gap-6 w-full px-4">
                        <div className="col-span-3 border-b-4 border-pink-100 pb-4 mb-4 flex items-center gap-4">
                          <XOctagon size={48} className="text-pink-500" />
                          <p className="text-5xl font-black text-pink-600">目前尚有缺交任務：</p>
                        </div>
                        {prevTasks.filter(t => !viewOnlyStudent.tasks[t]).map((t, idx) => (
                          <div key={idx} className="p-8 bg-pink-50 border-4 border-pink-200 rounded-[2.5rem] flex items-center gap-6 shadow-sm"><span className="text-4xl font-black text-pink-700">{t}</span></div>
                        ))}
                      </div>
                    )}
                  </div>
            ...
            {activeStudent && (
              <div className="grid grid-cols-3 gap-6 shrink-0 h-28 mt-8">
                <button onClick={() => submitCheckin('present')} className="bg-sky-600 text-white rounded-[2rem] text-4xl font-black shadow-xl hover:bg-sky-500 transition-all active:scale-95">確認打卡</button>
                <button onClick={() => submitCheckin('sick')} className="bg-slate-500 text-white rounded-[2rem] text-4xl font-black hover:bg-slate-600 transition-all shadow-md">病假</button>
                <button onClick={() => submitCheckin('personal')} className="bg-slate-600 text-white rounded-[2rem] text-4xl font-black hover:bg-slate-700 transition-all shadow-md">事假</button>
              </div>
            )}
          </div>
        </div>
