...
const APP_VERSION = "v10.0.260221_GiantFont_Stable";
...
const App = () => {
  ...
  const [fontSize, setFontSize] = useState(40); // 任務文字調整
  ...
  const submitCheckin = async (status = 'present') => {
    ...
    try {
      ...
      if (diff !== 0) {
        setDoc(doc(db, `/artifacts/class-5a-app/public/data/student_bank/${activeStudent.id}`), { bronze: increment(diff) }, { merge: true }).catch(e => console.error("存簿寫入失敗", e));
      }
  ...
  return (
    <div className="min-h-screen bg-[#F0F9FF] flex flex-col font-sans select-none overflow-x-hidden">
      {/* 頂部 Header：字體加大 2 號 */}
      <header className="bg-white border-b-2 border-sky-100 px-8 py-4 flex items-center justify-between shadow-sm sticky top-0 z-[100]">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-baseline gap-6 border-l-4 border-sky-100 pl-6">
        ...
        <div className="flex-1 flex justify-center items-center gap-2 overflow-x-auto px-10 scrollbar-hide">
          {recordedDates.map(d => (
            <button key={d} onClick={() => setViewDate(new Date(d))} 
              className={`px-4 py-2 rounded-xl text-xl font-black transition-all ${formatDate(viewDate) === d ? 'bg-sky-600 text-white shadow-md' : 'bg-sky-50 text-sky-400 hover:bg-sky-100'}`}>
        ...
        <div className="flex items-center gap-4">
          {user && <button onClick={() => window.confirm("確定要刪除今日紀錄？") && deleteDoc(doc(db, "announcements", formatDate(viewDate)))} className="p-3 text-red-400 hover:bg-red-50 rounded-xl transition-all"><Trash2 size={28}/></button>}
          <div className="flex bg-slate-100 p-2 rounded-2xl items-center shadow-inner">
            <button onClick={() => setViewDate(new Date(viewDate.setDate(viewDate.getDate() - 1)))} className="p-2 hover:bg-white rounded-xl transition-all"><ChevronLeft size={36}/></button>
            <span className="text-2xl font-black px-6">{formatDate(viewDate)}</span>
            <button onClick={() => setViewDate(new Date(viewDate.setDate(viewDate.getDate() + 1)))} className="p-2 hover:bg-white rounded-xl transition-all"><ChevronRight size={36}/></button>
          </div>
          {/* 自動日期新增按鈕 */}
          {user && <button onClick={() => setDoc(doc(db, "announcements", formatDate(viewDate)), { date: formatDate(viewDate), items: displayItems }, {merge:true})} className="p-3 bg-emerald-100 text-emerald-600 rounded-xl hover:bg-emerald-500 hover:text-white transition-all shadow-sm"><Plus size={28}/></button>}
        </div>
      </header>
      ...
      <main className="flex-1 flex overflow-hidden p-4 gap-0">
        ...
        <div style={{ width: `${width1}%` }} className="bg-white rounded-[2.5rem] shadow-sm p-4 flex flex-col border border-sky-50 overflow-hidden relative z-30">
          ...
          <div className="grid grid-cols-2 gap-3 flex-1 overflow-y-auto pr-1 custom-scrollbar">
            {STUDENTS.map(s => {
              ...
              return (
                <button key={s.id} onClick={() => handleStudentClick(s)} className={`h-22 rounded-[1.5rem] flex flex-col items-center justify-center transition-all border-b-4 active:border-b-0 active:translate-y-1 ${color}`}>
                  <span className="text-xs font-bold opacity-30">{s.id}</span>
        ...
        <div style={{ width: `${100 - width1 - width2}%` }} className="bg-[#0C4A6E] rounded-[2.5rem] shadow-lg p-8 text-white flex flex-col overflow-hidden z-10">
          <div className="flex justify-between items-center mb-6 border-b border-white/10 pb-4">
            ...
            <div className="flex items-center gap-3 bg-white/10 p-2 rounded-2xl shadow-inner">
          ...
          <div className="flex-1 bg-black/20 rounded-[2rem] p-8 overflow-y-auto custom-scrollbar border border-white/5 shadow-inner">
            {isEditing ? (
              ...
              <div className="font-bold space-y-6" style={{ fontSize: `${fontSize}px` }}>
                {displayItems.map((item, i) => (
                  <div key={i} className="flex items-start gap-6 animate-fade-in">
                    <span className="flex-shrink-0 w-12 h-12 bg-sky-500/80 rounded-full flex items-center justify-center text-white text-2xl shadow-md border-2 border-white/20">{i+1}</span>
      ...
      {/* 月度分析報表：字體加大 3 號、表頭凍結 */}
      <section className="mx-4 mb-6 bg-white rounded-[2rem] p-6 shadow-xl border-2 border-blue-50 max-h-[35vh] overflow-hidden flex flex-col shrink-0">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-4xl font-black text-sky-900 flex items-center gap-4 px-2"><Calendar size={40}/> 月度航行分析報表</h3>
          <div className="flex gap-2 bg-blue-50 p-1.5 rounded-2xl">
            {["2月", "3月", "4月", "5月", "6月", "7月"].map(m => (
              <button key={m} onClick={() => setActiveStatMonth(m)} className={`px-6 py-2 rounded-xl text-xl font-black transition-all ${activeStatMonth === m ? 'bg-sky-600 text-white shadow-md scale-105' : 'text-slate-400 hover:bg-white'}`}>{m}</button>
            ))}
          </div>
        </div>
        <div className="flex-1 overflow-auto rounded-2xl border-2 border-sky-100 relative custom-scrollbar">
          <table className="w-full text-center table-fixed border-collapse">
            <thead className="sticky top-0 z-40 bg-sky-700 text-white shadow-lg">
              <tr className="text-2xl font-black">
                <th className="p-8 bg-sky-800 border-r border-sky-600 sticky left-0 z-50 w-32">姓名</th>
                <th className="p-8 bg-cyan-600 border-r border-sky-500">出席狀況</th>
                <th className="p-8 bg-indigo-600">作業繳交狀況</th>
              </tr>
            </thead>
            <tbody className="divide-y-2 divide-sky-50">
              {STUDENTS.map(s => (
                <tr key={s.id} className="hover:bg-sky-50 transition-colors">
                  <td className="p-8 text-4xl font-black text-sky-900 border-r-2 border-sky-50 sticky left-0 z-10 bg-white">{maskName(s.name)}</td>
          ...
          <div className="bg-white rounded-[4rem] w-full max-w-5xl p-12 shadow-2xl relative flex flex-col max-h-[85vh] border-8 border-sky-100">
            ...
            {activeStudent && (
              <div className="grid grid-cols-3 gap-6 shrink-0">
                <button onClick={() => submitCheckin('present')} className="py-8 bg-sky-600 text-white rounded-[2.5rem] text-4xl font-black shadow-xl hover:scale-105 active:scale-95 transition-all">確認簽到領獎勵</button>
