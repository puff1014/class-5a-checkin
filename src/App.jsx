    ...
    <div className="min-h-screen bg-[#F0F9FF] flex flex-col font-sans select-none overflow-x-hidden">
        ...
        {showCalendarPicker && (
          <div className="absolute right-8 top-full mt-2 bg-white border-4 border-sky-200 rounded-[2rem] shadow-2xl z-[200] p-6 w-80 animate-in fade-in slide-in-from-top-4">
            <div className="flex justify-between items-center mb-4">
              <h4 className="text-2xl font-black text-sky-800">快速航行跳轉</h4>
              <button onClick={() => setShowCalendarPicker(false)} className="text-slate-300 hover:text-rose-500"><XCircle size={24}/></button>
            </div>
      ...
      <main className="flex flex-col lg:flex-row p-4 gap-2 print:hidden items-stretch pb-12">
        ...
        <div className="flex-1 bg-[#0C4A6E] rounded-[3rem] shadow-xl p-8 text-white flex flex-col shrink-0 min-w-0 relative overflow-hidden">
          ...
          <div className="flex-1 bg-black/20 rounded-[2.5rem] p-8 shadow-inner relative z-10 backdrop-blur-sm border border-white/5">
            {isEditing ? (
              <textarea value={announcementText} onChange={e => setAnnouncementText(e.target.value)} style={{ fontFamily: useBiauKai ? '"BiauKai", "DFKai-SB", "標楷體", serif' : 'inherit' }} className={`flex-1 bg-transparent text-white outline-none leading-relaxed text-4xl w-full min-h-[400px] ${useBiauKai ? 'font-normal' : 'font-black'}`} placeholder="輸入今日任務..." />
            ) : (
